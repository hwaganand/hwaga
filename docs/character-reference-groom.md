...
| `@char_MINJUN_knit_v1` | **크림 니트 크루넥 + 화이트 이너 + 차콜 슬랙스** | `b436b93b-bf9e-4951-a386-0202f23748d5` | **2화 6컷~ 전용** |
| `@char_MINJUN_hike_v1` | **형광 주황 등산 셸 + 그레이 정장 바지 + 브라운 더비** | `6d5eec88-7c34-4bb5-942c-133e1746c2cd` | **2화 1~5컷 전용** |

⚠️ **등산복 시트는 「못생긴 옷」이 아니라 「조합이 틀린 옷」이다.**
각 아이템을 제대로 된 고급품으로 지정했다(`no cheap or worn or dirty clothing`).
싸구려로 렌더링되면 그냥 추레한 사람이 되고, **각각이 멀쩡해야 조합이 웃긴다.**
확인 포인트 — 주황이 형광인가 / 바지에 주름선이 잡혔나 / 구두가 갈색 더비인가.
