...
## ⑥ 영상 비율 — 2화부터 9:16 세로 ★
배포처가 쓰레드·인스타·숏츠다. 가로로 올리면 화면 절반이 빈다.
1화는 16:9로 이미 생성됐다 — **리프레임하지 말고** 세로 캔버스 가운데에 놓고
위아래를 제목 카드로 채운다. 잘라내면 컷 6(손이 만나는 투샷)이 깨진다.

**세로에서는 투샷을 쓰지 않는다.** 대화 씬은 단독 샷을 교대로 자른다.
자막은 하단에서 15% 이상 띄운다 — 플랫폼 UI가 가린다.
...
인터뷰는 **두 자리로 나뉜다.** 2화의 교차 구조 그대로다.

| | 자리 |
|---|---|
| 준영 | **단독** (옷가게 쪽 사람) |
| 서연 + 지원 | **같은 자리 투샷** (서연 방 쪽 사람들) |

```
1) 준영 단독 — 회색 벽, 미디엄 샷
2) 서연 단독 — 아이보리 벽, 미디엄 클로즈업
3) 서연 + 지원 투샷 — 같은 벽, 지원이 몸을 기울이며 끼어든다
...
서연: …야.
```
⚠️ **준영과 서연은 서로의 말을 듣지 못한다.** 다른 인터뷰인데 **편집으로 붙여서
대화처럼 보이게** 하는 것이다. 다큐·예능의 표준 문법이라 관객이 바로 알아본다.
⚠️ **서연과 지원은 같은 자리에 있어야 한다.** `…야.` 는 지원에게 하는 말이고,
지원이 없으면 성립하지 않는다. **셋을 전부 따로 앉히면 개그가 깨진다.**
...
⚠️ **과거형이다.** 인터뷰는 결혼한 뒤 회상하는 시점이라, 현재형 「호감은 아니고요」는
...
| `@char_SEOYEON_ep2_v1` | **차콜 니트 + 미디 스커트 (2화 6컷~)** | job `85fa6407-6a8b-4adb-8352-8b41152f6523` | ✅ |
...
| `@char_JUNYOUNG_v1` | **준영 3분할 기본 (차콜 크루넥)** | job `0a52c9f5-6034-4961-ac87-5e0453fae32e` | ✅ |
...
| 민준 등산복 수정본 | `422d130d` 를 쓸지 미정 | 컷 2 |

✅ **레퍼런스 ID는 전부 확보됐다.** 2화 컷 생성을 막는 것은 없다.
