...
### 전제: 영상 생성 모델은 voice reference를 받지 않는다

Seedance 2.0 / WAN 3.0의 `generate_audio: true`는 **매 컷 새 목소리를 만든다.**
프롬프트에 "이 영상의 목소리를 참조하라"고 써도 반영되지 않는다.
20부작 내내 만식·순자·태수의 목소리를 고정하려면 아래 구조가 유일한 방법이다.

### 표준 절차

1. **Voice 등록(클론)** — 인물별로 깨끗한 단일 화자 음성 20~30초를 확보해 Higgsfield Voice로 등록.
   반환되는 `voice_id`(+ `voice_type: 'element'`)를 `ELEMENT_REGISTRY.md`에 기록한다.
2. **영상 생성** — 지금처럼 대사를 포함해 생성한다. 입 모양(립싱크)은 모델이 붙인 오디오 기준으로 만들어지므로
   이 단계에서 오디오를 끄면 안 된다.
3. **목소리 교체** — `voice_change(video_id, voice_id, voice_type='element')`로 영상 속 목소리만
   등록된 캐릭터 voice로 바꾼다. 타이밍과 화면은 그대로 유지된다.
4. **후반 합성** — 현장음·자막·ATM 숫자는 편집 단계에서 얹는다.

### 이 파이프라인이 강제하는 컷 설계 규칙

`voice_change`는 **영상 안의 모든 발화를 하나의 voice로 덮는다.**
따라서 한 컷 안에서 두 사람이 말하면 둘 다 같은 목소리가 된다.

→ **컷 설계 원칙을 "한 컷 한 화자"로 좁힌다.**
기존 규칙 "한 컷의 실제 화자는 최대 2명"은 화면에 두 명이 있어도 되지만
**대사를 치는 사람은 한 명**이어야 한다는 뜻으로 운용한다.
대화 장면은 화자별로 컷을 쪼갠다. (EP01 CUT01이 만식 대사 한 줄만 쓰는 설계인 것과 같은 논리)

### 클론 소스 품질 기준

- 길이 20~30초 이상, 한 사람만, 배경음악·효과음·상대 대사 없음
- 이미 만든 영상에서 뽑을 경우 발화 구간이 짧게 조각나므로,
  1차로 짧은 소스로 클론 → 그 voice로 30초짜리 깨끗한 독백을 TTS 생성 →
  그 결과물로 다시 클론하면 안정적인 정본 voice를 만들 수 있다.

### 실행 위치 주의

Higgsfield `create_voice`는 로컬 브라우저 위젯에서 파일을 직접 선택해야 한다.
**클라우드 세션(Claude Code on the web)에서는 채팅 첨부 파일을 Higgsfield로 넘길 수 없다.**
Voice 등록은 Claude Code 데스크톱 앱 또는 Higgsfield 웹에서 수행한다.
