...
## 8. 목소리 일관성 파이프라인 (Voice Consistency)

### 기본 방법: `audio_references`로 목소리를 직접 물린다

Seedance 2.0은 `medias` 배열에서 `role: "audio_references"`를 지원한다.
인물별 음성 파일을 첨부하면 그 음색·피치·나이감으로 대사가 발화된다.
**후처리 없이 생성 단계에서 목소리를 고정하는 것이 표준 경로다.**

| 지원 | 미지원 |
|---|---|
| `seedance_2_0`, `seedance_2_0_mini`, `minimax_h3`(H3), `minimax_h3_max`, `grok_video_v15` | `flux_3_video` |

목소리를 고정해야 하는 컷에는 미지원 모델을 쓰지 않는다.

### 절차

1. 인물별 음성 파일을 `drama/엄마의통장/voice/`에 둔다.
2. `media_upload` → PUT → `media_confirm`으로 **media_id(UUID)** 확보. `ELEMENT_REGISTRY.md`에 기록.
3. 컷 생성 시 `medias`에 `{ "role": "audio_references", "value": "<media_id>" }` 추가.
4. `generate_audio`는 `true` 유지 — `audio_references`와 독립이며, 꺼지면 현장음까지 사라진다.
5. 프롬프트 텍스트에는 **연기 방향만** 쓴다. 음색·피치는 텍스트가 아니라 첨부 음성이 결정한다.

> 주의: `audio_references`는 **첨부 미디어**다. 프롬프트 본문에 파일명이나
> `<<<...>>>` 태그만 써두면 적용되지 않는다.

### 보조 경로: `voice_change` (이미 만든 영상 수정용)

`voice_change(video_id, voice_id, voice_type='element')`는 완성된 영상의 목소리만
교체한다. 화면과 타이밍은 유지된다. 새 컷에는 위의 `audio_references`를 쓰고,
이 도구는 **이미 뽑아버린 컷의 목소리가 튀었을 때 되살리는 용도로만** 쓴다.

`voice_change`는 컷 안의 모든 발화를 하나의 voice로 덮으므로, 두 사람이 말하는
컷에는 쓸 수 없다.

### 컷 설계 원칙: 한 컷 한 화자

기존 규칙 "한 컷의 실제 화자는 최대 2명"은 다음으로 운용한다.
**화면에 두 명이 있어도 되지만, 대사를 치는 사람은 한 명.**
대화 장면은 화자별로 컷을 쪼갠다. 이유:

- `audio_references`에 서로 다른 두 목소리를 물리면 어느 대사에 어느 목소리가 갈지 통제할 수 없다
- `voice_change` 보조 경로가 아예 막힌다
- EP01 CUT01이 만식 대사 한 줄만 쓰는 설계인 것과 같은 논리

### 음성 소스 품질 기준

- 한 사람만, 배경음악·효과음·상대 대사 없음
- 길게 확보할수록 안정적이다. 기존 영상에서 뽑으면 발화 구간이 조각나므로,
  1차 결과가 흔들리면 그 목소리로 깨끗한 독백을 생성해 2차 소스로 교체한다.
- 현재 확보분: `voice/만식_젊은시절_voice_ref.mp3`(5.9초),
  `voice/순자_젊은시절_voice_ref.mp3`(2.2초) — 1979 공장 휴게실 컷에서 추출
